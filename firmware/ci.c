...
void ci_service(void)
{
	...
	if(strncmp(token, "dummy", 5) == 0) {
	  ...
	  zappio_hv_engage_write(1);  // engage the supply before writing, under the theory that the 
	  ...
	  int acq_timer, start_time;
	  monitor_depth_write(depth);
	  monitor_presample_write(200); // pull in 200 samples before engaging the zap
