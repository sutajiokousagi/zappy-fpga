...
/*===========================================================================*/
	...
	#define GDISP_INITIAL_CONTRAST	128
#endif
#ifndef GDISP_INITIAL_BACKLIGHT
	#define GDISP_INITIAL_BACKLIGHT	128
#endif
...
#define xybits(x, y, c)		((c)<<(( ((x)&1) ? 0 : 1) <<2))
...
LLDSPEC gBool gdisp_lld_init(GDisplay *g) {
    ...
    write_data(g,0xfd);//	0xfd,Enhanced low GS display quality;default is 0xb5(normal),

    write_cmd(g,CMD_SET_CONTRAST_CURRENT);
    write_data(g,0x80); // 0xff 	default is 0x7f
    ...
    write_data(g,0x82);	// Reserved;default is 0xa2(normal)
    write_data(g,0x20);
	...
	LLDSPEC void gdisp_lld_draw_pixel(GDisplay *g) {
		...
		*ram &= ~((uint8_t) xybits(x, y, LLDCOLOR_MASK()));
		*ram |= (uint8_t) xybits(x, y, gdispColor2Native(g->p.color));
		g->flags |= GDISP_FLG_NEEDFLUSH;
	}
#endif
